...
export default function Products() {
  ...
  // Active main tab ('breads' or 'drinks')
  ...
  const handleAddToCart = (product) => {
    ...
    const itemImage = product.image || product.img;
  ...
  // FIXED STRICT & HYBRID FILTER LOGIC FOR BREADS & DRINKS
  const filteredProducts = products.filter((p) => {
    ...
    if (activeTab === 'drinks') {
      ...
      // 1. Explicit SubCategory DB Match (Highest Priority)
      if (subCat) {
        return subCat === drinkFilter.toLowerCase();
      }

      // 2. Keyword Fallbacks (Only if subCategory is missing in DB)
  ...
  return (
    <>
      <style>{`
        ...
        .animated-grid {
          animation: gridFadeIn 0.45s ease-out forwards;
        }
        ...
        .transition-card:hover {
          ...
          box-shadow: 0 12px 24px rgba(43, 30, 22, 0.15);
        }
      ...
      <section className="menu-heading" style={{ textAlign: 'center', padding: '2.5rem 1rem 1rem' }}>
        ...
        {activeTab === 'breads' && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
            <button
              onClick={() => setBreadFilter('all')}
              className={`sub-filter-pill ${breadFilter === 'all' ? 'active' : ''}`}
            >
              All Breads
            </button>
            <button
              onClick={() => setBreadFilter('sweet')}
              className={`sub-filter-pill ${breadFilter === 'sweet' ? 'active' : ''}`}
            >
              🍯 Sweet
            </button>
            <button
              onClick={() => setBreadFilter('salty')}
              className={`sub-filter-pill ${breadFilter === 'salty' ? 'active' : ''}`}
            >
              🧀 Salty & Savoury
            </button>
            <button
              onClick={() => setBreadFilter('cakes')}
              className={`sub-filter-pill ${breadFilter === 'cakes' ? 'active' : ''}`}
            >
              🍰 Cakes
            </button>
          </div>
        ...
        {activeTab === 'drinks' && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
            <button
              ...
              className={`sub-filter-pill ${drinkFilter === 'all' ? 'active' : ''}`}
            >
              All Drinks
            </button>
            <button
              onClick={() => setDrinkFilter('coffee')}
              className={`sub-filter-pill ${drinkFilter === 'coffee' ? 'active' : ''}`}
            >
              ☕ Coffee
            </button>
            <button
              onClick={() => setDrinkFilter('tea-cold')}
              className={`sub-filter-pill ${drinkFilter === 'tea-cold' ? 'active' : ''}`}
            >
              🍵 Tea & Cold Drinks
            </button>
            <button
              onClick={() => setDrinkFilter('smoothies')}
              className={`sub-filter-pill ${drinkFilter === 'smoothies' ? 'active' : ''}`}
            >
              🥤 Smoothies
            </button>
          </div>
      ...
      {loading ? (
        <p style={{ textAlign: 'center', color: '#7a4f2e', padding: '2rem' }}>
          Loading products...
        </p>
        ...
        <div key={`${activeTab}-${breadFilter}-${drinkFilter}`} className="tab-panel active animated-grid">
