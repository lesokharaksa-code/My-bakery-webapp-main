...
export default function Service() {
  ...
  const handleSubmitPartyOrder = async (e) => {
    ...
    try {
      // Connects directly to admin orders collection
      await addDoc(collection(db, 'orders'), {
        ...
        customRequirements: {
          loavesCount: Number(loaves),
          pastriesCount: Number(pastries),
          drinksCount: Number(drinks),
        },
        notes,
      ...
      alert('Failed to send order inquiry. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: '#FFFFFF', padding: '2.5rem', borderRadius: '12px', border: '1px solid #E5E7EB', maxWidth: '700px', margin: '0 auto', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          <p style={{ color: '#666', fontSize: '0.95rem', marginBottom: '1.5rem' }}>
            Selected Package: <strong style={{ color: '#D97706' }}>{selectedPack}</strong>
          </p>
          ...
          {submitted ? (
            <div style={{ backgroundColor: '#ECFDF5', color: '#047857', padding: '1.5rem', borderRadius: '8px', textAlign: 'center' }}>
              ...
              <p style={{ margin: 0 }}>We received your request and your inquiry is now in the admin approval queue.</p>
              <button 
                onClick={() => setSubmitted(false)} 
                style={{ marginTop: '1rem', background: '#047857', color: '#fff', border: 'none', padding: '0.5rem 1rem', borderRadius: '4px', cursor: 'pointer' }}
              >
            ...
            <form onSubmit={handleSubmitPartyOrder}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.3rem', fontWeight: 'bold', color: '#374151' }}>Phone Number</label>
                  <input
                    type="tel"
                    value={phone}
              ...
              <h4 style={{ margin: '1.5rem 0 0.8rem 0', color: '#2B1E16', fontSize: '1.1rem' }}>Customize Quantities</h4>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.3rem', fontWeight: 'bold', color: '#374151' }}>Special Instructions / Notes</label>
                <textarea
                  rows="3"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Dietary requirements, flavor preferences, delivery details..."
                  style={{ width: '100%', padding: '0.65rem', borderRadius: '6px', border: '1px solid #D1D5DB', boxSizing: 'border-box', fontFamily: 'sans-serif' }}
                />
              </div>
              ...
              >
                {loading ? 'Submitting Order Inquiry...' : 'Send Party Order Request'}
              </button>
            </form>
          )}
        </div>
